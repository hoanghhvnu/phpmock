		...
		if ($this->input->post ( "ok" )) {
			...
			if ($this->form_validation->run ()) {
				...
				foreach ( $listall as $row ) {
					if (in_array ( trim ( $updateName ), $row ) && ($row ['pro_id'] != $id))
						$data ['errorName'] = "Da ton tai";
				}
				if ($_FILES ['file'] ['error'] == 0 && $numberImages >= 10)
					$data ['errorImages'] = "Da du 10 anh";
				else if (! isset ( $data ['errorName'] )) {
					$cate = array ();
	...
	public function insertProduct() {
		...
		if ($this->input->post ( "ok" )) {
			...
			if ($this->form_validation->run ()) {
				...
				$listall = $this->product_model->getAll ();
				foreach ( $listall as $row ) {
					if (in_array ( trim ( $updateName ), $row ))
						$data ['errorName'] = "Da ton tai";
				} // end if in_aaray
